$(document).ready(function() {
  var $svg = $(".sidebar"),
      ...
      animating = false;
  $svg.get(0).setAttribute("viewBox", "0 0 "+clientWidth+clientHeight);
 var path_d="M0,0 "+clientWidth/6 +",0 a0,"+clientWidth*0.66+" 0 1,1 0,"+clientHeight+" L0,"+clientHeight;
  $path.get(0).setAttribute("d",path_d)
  var easings = {
  ...
  function createD(top, ax, dir) {
    return "M0,0 "+top+",0 a"+ax+","+clientWidth*0.7+" 0 1,"+dir+" 0,"+clientHeight+" L0,"+clientHeight;
  }

  var startD = createD(50,0,1),
      midD = createD(125,75,0),
      finalD = createD(clientWidth*0.66,0,1),
      clickMidD = createD(clientWidth,80,0),
      clickMidDRev = createD(200,100,1),
      clickD = createD(clientWidth,0,1),
      currentPath = startD;
  ...
  function moveImage(that) {
    ...
    $demo.append($clone);
    $clone.css("top");
    $clone.css({top: "0.5rem", left: "9.5rem"});
  }
